...
class CosmicEngine:
    VERSION = "v40.0 Masterclass Legacy"
    STRATEGY = [
...
# --- 2. THE SUPREME VISUAL INTERFACE (BACK TO MASTERCLASS STYLE) ---
st.set_page_config(page_title="MASTERCLASS LEGACY v40.0", layout="wide")
...
st.markdown("<h1 class='glow-title'>✨ THE MASTERCLASS LEGACY v40.0 ✨</h1>", unsafe_allow_html=True)
st.markdown("---")
...
# --- 4. TAMPILAN AWAL (KEMBALI KE GAYA MASTERCLASS ASLI) ---
if not clean_data:
    ...
    with col_guide_1:
        ...
        <p><span class='step-number'>3</span> <b>Sinkronisasi AI:</b> Cek Cuaca Mesin dan Grafik 3D untuk konfirmasi.</p>
        <p><span class='step-number'>4</span> <b>Eksekusi Modal:</b> Gunakan BBFS sesuai rekomendasi unit pasang.</p>
        """, unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

    with col_guide_2:
        st.markdown("<div class='master-box' style='border-color: #ffd700;'>", unsafe_allow_html=True)
        st.subheader("⚡ Teknologi Entitas Terintegrasi")
...
# --- 5. LOGIKA & OUTPUT (THE UNIFIED LEGACY) ---
else:
    try:
        ...
        t1, t2, t3, t4, t5, t6 = st.tabs([
            "🎯 KEPUTUSAN MUTLAK", "🌪️ CUACA MESIN", "📊 GRAFIK 3D CLUSTER", "🔬 BEDAH RUMUS", "🛡️ QUANTUM BBFS", "💰 KEUANGAN"
        ])

        with t1:
            st.subheader("🏆 Hasil Prediksi & Bedah Anatomi")
            c1, c2, c3 = st.columns(3)
            with c1: st.markdown(f"<div class='god-card'><h4>2D CORE</h4><h1 style='font-size:70px; color:#ffd700;'>{ai_final[2:]}</h1></div>", unsafe_allow_html=True)
            with c2: st.markdown(f"<div class='god-card'><h4>3D CORE</h4><h1 style='font-size:70px; color:#ffd700;'>{ai_final[1:]}</h1></div>", unsafe_allow_html=True)
            with c3: st.markdown(f"<div class='god-card'><h4>4D CORE</h4><h1 style='font-size:70px; color:#ffd700;'>{ai_final}</h1></div>", unsafe_allow_html=True)
        ...
        with t2:
            ...
            st.write(f"**Laporan Teknis:** Mesin saat ini menunjukkan bias pada digit **{res_gap}**. Pola pengacakan terlihat lemah bagi AI karena repetisi pada posisi KOP sudah mencapai ambang batas prediksi.")
        ...
        with t3:
            ...
            else: st.warning("Grafik 3D membutuhkan library 'plotly'.")
        ...
        with t6:
            ...
            st.markdown("**Aturan Emas:** Disiplin pada target profit. Berhenti saat cuaca berubah menjadi BADAI.")

    except Exception as e:
        st.error(f"⚠️ Kesalahan Sinkronisasi: {e}")

st.markdown("---")
st.caption(f"© 2026 {CosmicEngine.VERSION} | The Original Masterclass Restored")
